		...
		for(int i=startIndex; i<=endIndex; i++) {
			...
			for(CCNBBill currentRecord: unmigratedRecords) {				
				try {
					...
					bill.setLoadFactorIncentive(new BigDecimal(currentRecord.getLoadFactorIncentive()).abs());
					bill.setSdInterest(new BigDecimal(currentRecord.getSdInterest()).abs());
					...
					bill.setEmployeeRebate(new BigDecimal(currentRecord.getEmployeeRebate()).abs());
					bill.setOnlinePaymentRebate(new BigDecimal(currentRecord.getOnlinePaymentRebate()).abs());
					bill.setPrepaidMeterRebate(new BigDecimal(currentRecord.getPrepaidMeterRebate()));
					bill.setPromptPaymentIncentive(new BigDecimal(currentRecord.getPromptPaymentIncentive()).abs());
					bill.setAdvancePaymentIncentive(new BigDecimal(currentRecord.getAdvancePaymentIncentive()).abs());
					bill.setDemandSideIncentive(new BigDecimal(currentRecord.getDemandSideIncentive()));
					bill.setSubsidy(new BigDecimal(currentRecord.getSubsidy()).abs());
