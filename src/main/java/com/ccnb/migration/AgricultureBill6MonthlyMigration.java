...
public class AgricultureBill6MonthlyMigration {
	...
	public static void main(String[] args) throws Exception {
		...
		File file = new File(PathUtil.baseExceptionFolder + "AgricultureBill6MonthlyMigrationExceptionLog.txt");
		FileWriter fw = null;
		...
		Query<String> billMonthQuery = session.createQuery("select distinct(startBillMonth) from CCNBAgricultureBill6Monthly");
		List<String> billMonths = billMonthQuery.list();

		List<Thread> runningThreads = new ArrayList<>();
		List<Agriculture6MonthlyBillMigrationWorker> workerObjects = new ArrayList<>();
		...
		while (temp > 0) {
			...
			if (div > 0) {
				...
				Agriculture6MonthlyBillMigrationWorker worker = new Agriculture6MonthlyBillMigrationWorker(sessionFactory, curIndex, curIndex + div - 1, billMonths, writer);
		...
		for (Agriculture6MonthlyBillMigrationWorker worker : workerObjects) {
		...
		System.out.println("MIGRATION FROM CCNB_AGRICULTURE_6_MONTHLY SUCCESSFULLY DONE !!");
